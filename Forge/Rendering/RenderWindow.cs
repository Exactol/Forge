...
namespace Forge
{
    ...
    sealed partial class MainWindow : GameWindow
    {
        ...
        private readonly string _baseDirectory = Path.GetDirectoryName(Path.GetDirectoryName(Path.GetDirectoryName(Assembly.GetExecutingAssembly().CodeBase).TrimEnd(Path.DirectorySeparatorChar)).TrimEnd(Path.DirectorySeparatorChar)).TrimEnd(Path.DirectorySeparatorChar);
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            _shadedShader.Add(_baseDirectory + "/Rendering/Shaders/normalVertexShader.vert");
            _shadedShader.Add(_baseDirectory + "/Rendering/Shaders/normalFragmentShader.frag");
            _shadedShader.Add(_baseDirectory + "/Rendering/Shaders/normalGeometryShader.geom");
            ...
            //renderObjects.Add(new RenderObject(MeshFactory.PlaneLumpToVertices(BSP.planeList[0]), true));
            ...
            renderObjects.Add(new RenderObject(MeshFactory.FaceLumptoVertex(temp[0])));
        ...
        protected override void OnUpdateFrame(OpenTK.FrameEventArgs e)
        {
            ...
            if (camActive)
            {
                cam.HorizontalAngle += (Mouse.X - (Width / 2)) * e.Time * cam.mouseSpeed;
                cam.VerticalAngle += -(Mouse.Y - (Height / 2)) * e.Time * cam.mouseSpeed;
                CenterMouse();
            }
            ...
            cam.viewMatrix = Matrix4.LookAt(cam.Position, cam.Position + Vector3.Normalize(cam.front), cam.up );
        }
        ...
        {
            Title = $"Forge {versionNum} FPS: {1f / e.Time:0} Map: {BSP.mapName}";
